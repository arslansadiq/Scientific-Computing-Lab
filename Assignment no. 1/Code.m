...
%%%%%%%%%%%%       Implementation of Explicit Heun Method's from Worksheet 2      %%%%%%%%%%%%%

exact_error_EH = zeros(1 , size(time_step , 2));
for i=1:size(time_step , 2)
    heun_exp_approx = explicit_heun(time_step(i) , time_end , initial_condition);
    figure(2)
    axis([0 5 0 20]);
    title('Numerical Approximation Method: Explicit Heun Method');
...
plot(time , analytical_solution , 'DisplayName' , 'Analytical Solution');
legend show
Explicit_Heun_Table = array2table(Error_Table(exact_error_EH) , 'RowNames' , {'dt','error','error red'})
...
%%%%%%%%%%%          Explicit Heun Method's Definition        %%%%%%%%%%%%%

function approx_value = explicit_heun(dt , time_end , y0)
approx_value = zeros(1 , (time_end./dt)+1);
approx_value(1) = y0;
for i = 1:(size(approx_value , 2)-1)
...
error = 5;
initial_guess = yi_1+25;
itirations=0;
while (error >= accuracy_limit)&& itirations < 100
